...
node *createleftclone(node *root)
{
    ...
    node *leftclonedroot = createleftclone(root->left);
    node *rightleftclonedroot = createleftclone(root->right);

    node *newnode = new node(root->data, leftclonedroot, NULL);
    root->left = newnode;
    root->right = rightleftclonedroot;
